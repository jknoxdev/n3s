...
def sanitize_filenames(folder_path):
    summaries_folder_path = r'C:\Users\Administrator\Desktop\jpeg-design\reports\scripts\summaries'
    status_folder_path = r'C:\Users\Administrator\Desktop\jpeg-design\reports\scripts\status'
    ...
    for file_name in os.listdir(summaries_folder_path):
        if file_name.endswith(".pdf"):
            ...
            if sanitized_name.count("_") > 2:
                sanitized_name = sanitized_name.replace("_", "", 1)
            elif sanitized_name.count("_") > 1:
                sanitized_name = sanitized_name.replace("_", "_", 1)
            elif sanitized_name.count("_") == 0:
                sanitized_name = sanitized_name.replace(" ", "_")
            elif sanitized_name.count("_") == 1:
                sanitized_name = sanitized_name.replace("_", "")

            #check for too many dashes
            if sanitized_name.count("-") > 2:
                sanitized_name = sanitized_name.replace("-", "", 1)
            elif sanitized_name.count("-") > 1:
                sanitized_name = sanitized_name.replace("-", "_", 1)
            ...
            sanitized_name = sanitized_name.replace("__", "_")
            sanitized_name = sanitized_name.lower() #lowercase

            print("......",file_name,".....to......", sanitized_name, "......")
            os.rename(
    ...
    for file_name in os.listdir(status_folder_path):
        if file_name.endswith(".pdf"):
            ...
            if os.path.exists(os.path.join(status_folder_path, f"{sanitized_name}.pdf")):
                print(f"File {sanitized_name}.pdf already exists. Skipping...")
            else:
                os.rename(
...
#TODO: Add a function to check if the merged PDFs are the same as the original PDFs
#TODO: prompt user to select the root directires of the summaries and status folders
folder_path = r'C:\Users\Administrator\Desktop\jpeg-design\reports\scripts'
